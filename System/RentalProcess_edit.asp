...
if juhaoyong_a_pid<>a_pid then
	if a_pid<>"" then call RentalProcess_to_html(a_pid)
	if juhaoyong_a_pid<>"" then call RentalProcess_to_html(juhaoyong_a_pid)
else
	if a_pid<>"" then call RentalProcess_to_html(a_pid)
end if

if juhaoyong_a_ppid<>a_ppid then
	if a_ppid<>"" then call RentalProcess_to_html(a_ppid)
	if juhaoyong_a_ppid<>"" then call RentalProcess_to_html(juhaoyong_a_ppid)
else
	if a_ppid<>"" then call RentalProcess_to_html(a_ppid)
end if
...
response.Write "<script language='javascript'>alert('修改成功！');location.href='RentalProcess_list.asp?ppid="&juhaoyong_ppid&"&page="&page&"&act="&act&"&keywords="&keywords&"';</script>"
elseif juhaoyong_pid>0 then
response.Write "<script language='javascript'>alert('修改成功！');location.href='RentalProcess_list.asp?pid="&juhaoyong_pid&"&page="&page&"&act="&act&"&keywords="&keywords&"';</script>"
elseif juhaoyong_cid>0 then
response.Write "<script language='javascript'>alert('修改成功！');location.href='RentalProcess_list.asp?cid="&juhaoyong_cid&"&page="&page&"&act="&act&"&keywords="&keywords&"';</script>"
end if
...
return true;}
    ...
    <table cellpadding='3' cellspacing='1' border='0' class='tableBorder' align="center">
        <tr>
            <th class='tableHeaderText' colspan="2" height="25">修改租车流程</th>
...
sqlClass2="select id,pid,ppid,name from category where ppid=2 and ClassType=7 and pid="&cint(rs("cid"))&" order by id" 
...
sqlClass3="select id,pid,ppid,name from category where ppid=3 and ClassType=7 and pid="&cint(rs("pid"))&" order by id" 
