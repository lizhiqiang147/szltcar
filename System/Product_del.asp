...
if request("action")="AllDel" then
	...
	sql="select id,cid,pid,ppid,file_path,image from Article where id="&cint(article_id(i))&""
	rs.open(sql),cn,1,3
	...
	'先判断文件是否存在，否则删除
	FilePath=Server.MapPath(ProductContent_FolderName&"/"&FilePath)
	...
	set rs=server.createobject("adodb.recordset")
	sql="select id,cid,pid,ppid,file_path,image from article where id="&article_id&""
	rs.open(sql),cn,1,3
	FilePath=rs("file_path")
	imageName=rs("image")
	ClassID=rs("cid")
