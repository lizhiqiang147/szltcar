...
if juhaoyong_a_cid<>a_cid then
	if a_cid<>"" then call DrinkDrive_to_html(a_cid)
	if juhaoyong_a_cid<>"" then call DrinkDrive_to_html(juhaoyong_a_cid)
else
	if a_cid<>"" then call DrinkDrive_to_html(a_cid)
end if

if juhaoyong_a_pid<>a_pid then
	if a_pid<>"" then call DrinkDrive_to_html(a_pid)
	if juhaoyong_a_pid<>"" then call DrinkDrive_to_html(juhaoyong_a_pid)
else
	if a_pid<>"" then call DrinkDrive_to_html(a_pid)
end if

if juhaoyong_a_ppid<>a_ppid then
	if a_ppid<>"" then call DrinkDrive_to_html(a_ppid)
	if juhaoyong_a_ppid<>"" then call DrinkDrive_to_html(juhaoyong_a_ppid)
else
	if a_ppid<>"" then call DrinkDrive_to_html(a_ppid)
end if
...
return true;}
    ...
    <table cellpadding='3' cellspacing='1' border='0' class='tableBorder' align="center">
        <tr>
            <th class='tableHeaderText' colspan="2" height="25">修改酒后代驾</th>
