...
'       ppid    ----  三级目录id
'**************************************************
sub showpage_AritcleProduct(sfilename,totalnumber,maxperpage,ShowTotal,ShowAllPages,strUnit,cid,pid,ppid)
	dim n, i,strTemp,strUrl
	if totalnumber mod maxperpage=0 then
    	n= totalnumber \ maxperpage
  	else
	...
	strUrl=JoinChar(sfilename)
  	if page<2 then
    		...
    		strTemp=strTemp & "<a href='"&strUrl&"cid="&cid&"&pid="&pid&"&ppid="&ppid&"&act="&act&"&keywords="&keywords&"&page=1 '>首页</a>&nbsp;"
    		strTemp=strTemp & "<a href='"&strUrl&"cid="&cid&"&pid="&pid&"&ppid="&ppid&"&page="&(Page-1)&"&act="&act&"&keywords="&keywords&"'>上一页</a>&nbsp;"
  	end if

  	if n-page<1 then
    		strTemp=strTemp & "下一页 尾页"
  	else
    		strTemp=strTemp & "<a href='"&strUrl&"cid="&cid&"&pid="&pid&"&ppid="&ppid&"&page="&(Page+1)&"&act="&act&"&keywords="&keywords&"'>下一页</a>&nbsp;"
    		strTemp=strTemp & "<a href='"&strUrl&"cid="&cid&"&pid="&pid&"&ppid="&ppid&"&page="&n&"&act="&act&"&keywords="&keywords&"'>尾页</a>"
  	end if
   	strTemp=strTemp & "&nbsp;页次：<strong><font color=red>" & Page & "</font>/" & n & "</strong>页 "
    strTemp=strTemp & "&nbsp;<b>" & maxperpage & "</b>" & strUnit & "条/页"
	if ShowAllPages=True then
act="act="&request("act")
keywords="keywords="&request("keywords")
		strTemp=strTemp & "&nbsp;转到：<select name='page' size='1' onchange=""javascript:window.location='"&strUrl&"cid="&cid&"&pid="&pid&"&ppid="&ppid&"&"&act&"&"&keywords&"&"& "page=" & "'+this.options[this.selectedIndex].value;"">"   
    	for i = 1 to n   
    		strTemp=strTemp & "<option value='" & i & "'"
			if cint(Page)=cint(i) then strTemp=strTemp & " selected "
			strTemp=strTemp & ">第" & i & "页</option>"   
