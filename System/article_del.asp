...
if request("action")="AllDel" then
	...
	sql="select id,cid,pid,ppid,file_path,Files from article where id="&cint(article_id(i))&""
	rs.open(sql),cn,1,3
	...
	'先判断文件是否存在，否则删除
	FilePath=Server.MapPath(ArticleContent_FolderName&"/"&FilePath)
	If fso.FileExists(FilePath) then fso.DeleteFile(FilePath)
	
	'先判断附件是否存在，否则删除
	FilePath=Server.MapPath("../attachmentUploadFile/"&FilesName)
	If fso.FileExists(FilePath) then fso.DeleteFile(FilePath)
	...
	sql="select id,cid,pid,ppid,file_path,Files from article where id="&article_id&""
	rs.open(sql),cn,1,3
	FilePath=rs("file_path")
	FilesName=rs("Files")
	ClassID=rs("cid")
