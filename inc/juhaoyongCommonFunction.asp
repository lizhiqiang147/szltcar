...
'函数功能：获取一级栏目文件夹名称
function juhaoyongGetTopCategoryFolderName(categoryId)
set JHYCommon001rs=server.createobject("adodb.recordset")
sql="select [folder],[id] from [category] where id="&categoryId
JHYCommon001rs.open(sql),cn,1,1
if not JHYCommon001rs.eof then
	juhaoyongGetTopCategoryFolderName=JHYCommon001rs("folder")
else
	juhaoyongGetTopCategoryFolderName=""
end if
JHYCommon001rs.close
set JHYCommon001rs=nothing
end function
%>
