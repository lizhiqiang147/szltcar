...
juhaoyongArticleContentPid=0
juhaoyongArticleContentPPid=0
class_position=""
...
juhaoyongArticleContentPid=cint(ClassID)
juhaoyongArticleContentPPid=0
class_position=""
...
rs_1.open(sql),cn,1,1
if not rs_1.eof then
juhaoyongArticleContentPid=rs_1("id")
juhaoyongArticleContentPPid=cint(ClassID)
...
if rsl("id")=juhaoyongArticleContentPid then
...
Block_LeftClassList=Block_LeftClassList&"<li><A href='/"&ClassFolder1&"/"&rsl("Folder")&"'>"&rsl("name")&"</A> "
	...
	set rs=server.createobject("adodb.recordset")
	sql="select [id],[name],[folder] from [category] where ppid=3 and pid="&rsl("id")&" order by [order] "
	rs.open(sql),cn,1,1
	if not rs.eof then
	Block_LeftClassList=Block_LeftClassList&"<ul>"
	do while not rs.eof
		if rs("id")=juhaoyongArticleContentPPid then
		Block_LeftClassList=Block_LeftClassList&"<li class='current3juhaoyong'><a href='/"&ClassFolder1&"/"&rsl("Folder")&"/"&rs("folder")&"/' >"&rs("name")&"</a></li> "
		else
...
<%
 '文章列表读取替换
sql="select * from [article] where "&ClassSQL&"='"&ClassID&"' and view_yes=1  and ArticleType=2 order by [edit_time] desc"
set rs1=server.createObject("ADODB.Recordset")
rs1.open sql,cn,1,1
%><%
if not rs1.eof then
...
if rs("url")<>"" then
	...
	list_block=list_block&"<div class='inner'><a href='"&rs_url&"' target='_blank'><img src='/images/up_images/"&rs("image")&"' /></a></div>"
	list_block=list_block&"<div class='albumtitle'><a href='"&rs_url&"' target='_blank'>"&rs("title")&"</a></div>"
	if rs("net")<>"" then
	list_block=list_block&"<DIV class='boxPrice'>&#165;"&rs("net")&"</DIV>"
	end if
