...
<%
 '文章列表读取替换
sql="select [title],[content],[url],[keywords],[author],[file_path],[edit_time],[comment],[hit] from [article] where "&ClassSQL&"='"&ClassID&"' and view_yes=1  and ArticleType=1 order by [edit_time] desc"
set rs1=server.createObject("ADODB.Recordset")
rs1.open sql,cn,1,1
%><%
if not rs1.eof then
...
sql="select [id],[title],[content],[url],[keywords],[author],[cid],[file_path],[edit_time],[comment],[hit] from [article] where "&ClassSQL&"='"&ClassID&"' and view_yes=1  and ArticleType=1 order by [edit_time] desc"
set rs=server.createObject("ADODB.Recordset")
rs.open sql,cn,1,1
if not rs.eof then
...
list_block=list_block&"<td width='88%' class='fw_t'>·<a href='"&rs_url&"' target='_blank'>"&left(rs("title"),47)&"</a></td>"
list_block=list_block&"<td width='12%' class='fw_s' align=right>["&year(rs("edit_time"))&"-"&month(rs("edit_time"))&"-"&day(rs("edit_time"))&"]</td>"
list_block=list_block&"</tr>"
%>
<%
rs.movenext
